$( function() {
	...
	qt.Room = class {
		...
		draw( rows, indent ) {
			...
			if ( this.object && this.object.character ) {

				var cat = $( '<img>' ).attr( 'src', 'lib/img/' + this.object.character + '.svg' );
				...
				setInterval( () => {
					...
					cat.attr( 'src', 'lib/img/' + this.object.character + ( alt ? '2' : '1' ) + '.svg' );
					alt = !alt;
			...
			new qt.Room( {
				...
				new qt.Corridor( [
					...
					new qt.Room( '', [
						...
						new qt.Corridor( [ new qt.Room(), new qt.Room( 'lever2' ), new qt.Room( 'key1' ) ] ),
						new qt.Corridor( [ new qt.Room( 'spikes1' ), new qt.Room( '', [ new qt.Corridor( [ new qt.Room( 'heart1' ) ] ) ] ) ] ),
